...
dessinerFougere:             // INFO: dessinerFougere
    ...
    ucvtf d0,x3 
    ucvtf d1,x4
    adr x24,qOfqGraine       // adresse de la graine
    ldr x1,[x24]
    add x24,x24,x1
    ldr x24,[x24]            // charge la graine de départ
    ldr x10,qVal1
    ldr x11,qVal2
    ldr d5,dCent
    ldr d6,dConst2          // 0,85
    ...
    ldr d10,dConst5          // 1,6
    ldr d11,dConst6          // 0,2
    ldr d12,dConst7          // 0,26
    ldr d13,dConst8          // 0,23
    ldr d14,dConst9          // 0,22
    ldr d15,dConst10        // 0,15
    ldr d16,dConst11        // 0,28
    ldr d17,dConst12        // 0,24
    ldr d18,dConst13        // 0,44
    mov x7,100
    ldr x23,iNbpoints
1:                           // dans la boucle calcul de x3 (position X) et x4 (position Y)
    ...
    mul x24,x10,x24           // multiplie graine par qVal1
    add x24,x24,x11           // ajout de qVal2
    udiv x2,x24,x7           // division par 100
    msub x0,x2,x7,x24        // calcul resultat modulo plage
    
    cmp x0,1
    bgt 2f
                             // cas pour un % des cas
    fmov d0,d7                // x = 0
    fmul  d1,d1,d8           //  y * 0,16
    b 10f
2:
    cmp x0,86
    bgt 3f
                            // cas pour 85% des cas
    fmul  d3,d0,d6          // * x * 0,85
    fmul  d4,d1,d9          // y * 0,04
    fadd d3,d3,d4           // nouveau x
    fmul  d4,d0,d9          // x * 0,04
    fmul d1,d1,d6           // y * 0,85
    fsub d1,d1,d4           //   y * 0,85 - x * 0,04
    fadd d1,d1,d10           // y + 1,6 = nouveau y
...
3:
    ...
    fmul d3,d0,d11           // x * 0,2
    fmul  d4,d1,d12          // y * 0,26
    fsub d3,d3,d4           // nouveau x
    fmul  d4,d0,d13          // x * 0,23
    fmul  d1,d1,d14          // y * 0,22
    fadd  d1,d1,d4
    fadd d1,d1,d10          // y + 1,6
    fmov d0,d3
    b 10f
4:                         // reste des cas
    fmul  d3,d0,d15         // x * 0,15
    fmul  d4,d1,d16         // y * 0,28
    fsub d3,d4,d3          // nouveau x = -15x + 28 y
    fmul  d4,d0,d12         // x * 0,26
    fmul  d1,d1,d17         // y * 0,24
    fadd  d1,d1,d4
    fadd d1,d1,d18          // nouveau y = y * 0,44
    fmov d0,d3             // nouveau x
